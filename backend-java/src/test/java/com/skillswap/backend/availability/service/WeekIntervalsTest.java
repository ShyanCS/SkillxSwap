...
class WeekIntervalsTest {
    ...
    void utcSlotIsUnchanged() {
        List<WeekIntervals.Interval> projected = WeekIntervals.project(
                List.of(slot(1, 9, 0, 11, 0)), ZoneId.of("UTC"), WINTER_WEEK);
    ...
    void slotAheadOfUtcShiftsBack() {
        ...
        List<WeekIntervals.Interval> projected = WeekIntervals.project(
                List.of(slot(1, 18, 0, 20, 0)), ZoneId.of("Asia/Kolkata"), WINTER_WEEK);
    ...
    void crossTimezoneOverlapIsComputedOnRealInstants() {
        ...
        List<WeekIntervals.Interval> kolkata = WeekIntervals.project(
                List.of(slot(1, 18, 0, 20, 0)), ZoneId.of("Asia/Kolkata"), WINTER_WEEK);
        List<WeekIntervals.Interval> newYork = WeekIntervals.project(
                List.of(slot(1, 9, 0, 11, 0)), ZoneId.of("America/New_York"), WINTER_WEEK);
    ...
    void sameLocalHoursDifferentZonesDoNotOverlap() {
        ...
        List<WeekIntervals.Interval> kolkata = WeekIntervals.project(
                List.of(slot(1, 9, 0, 11, 0)), ZoneId.of("Asia/Kolkata"), WINTER_WEEK);
        List<WeekIntervals.Interval> newYork = WeekIntervals.project(
                List.of(slot(1, 9, 0, 11, 0)), ZoneId.of("America/New_York"), WINTER_WEEK);
    ...
    void slotCrossingWeekEndIsSplitNotTruncated() {
        ...
        List<WeekIntervals.Interval> projected = WeekIntervals.project(
                List.of(slot(7, 15, 0, 18, 0)), ZoneId.of("America/Los_Angeles"), WINTER_WEEK);
        ...
        assertThat(projected.stream().mapToInt(WeekIntervals.Interval::length).sum()).isEqualTo(180);
    }
    ...
    void slotBeforeWeekStartWraps() {
        ...
        List<WeekIntervals.Interval> projected = WeekIntervals.project(
                List.of(slot(1, 0, 0, 2, 0)), ZoneId.of("Asia/Kolkata"), WINTER_WEEK);

        int sundayEighteenThirty = 6 * 1440 + 18 * 60 + 30;
        assertThat(projected).containsExactly(
                new WeekIntervals.Interval(sundayEighteenThirty, sundayEighteenThirty + 120));
    }
    ...
    void coverageRequiresTheEntireRange() {
        List<WeekIntervals.Interval> available = WeekIntervals.project(
                List.of(slot(1, 9, 0, 11, 0)), ZoneId.of("UTC"), WINTER_WEEK);
